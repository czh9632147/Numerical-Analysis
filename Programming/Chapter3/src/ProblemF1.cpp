...
// Function to calculate divided differences recursively(already maltiplied by the denominator)
...
void write_gnuplot_script(const std::string& base_filename, const std::string& output_file, size_t num_truncated, size_t num_diff1, size_t num_diff2) {
    std::ofstream script("n=1_plot_script.gp");
    script << "set terminal pngcairo size 1200,800\n";
    script << "set output '" << output_file << "'\n";
    script << "set multiplot layout " << num_truncated << ",3 title 'n=1: Truncated Power Functions and Divided Differences'\n";
    script << "set xlabel 'x'\n";
    script << "set ylabel 'Value'\n";
    script << "set xrange [-1:5]  # Set the x-axis range uniformly\n";
    script << "set yrange [-0.5:1.5]  # Set the y-axis range uniformly\n";
...
int main() {
    ...
    write_gnuplot_script("basis_n1", "basis_functions_n1.png", knots.size() - 1, knots.size() - 2, knots.size() - 3);

    // Call Gnuplot to generate plots
    int result = std::system("gnuplot n=1_plot_script.gp");
    if (result != 0) {
