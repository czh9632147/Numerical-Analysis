...
void generateCurve(const std::vector<double>& tPoints, std::vector<double>& xPoints, 
    ...
    for (double t : tPoints) {
        double x = std::sin(std::cos(2 * t)) * std::cos(t);
        double y = std::sin(std::cos(2 * t)) * std::sin(t);
        double z = -std::cos(std::cos(2 * t));
        xPoints.push_back(x);
        yPoints.push_back(y);
        zPoints.push_back(z);
    }
...
int main() {
    ...
    for (int numPoints : numPointsArray) {
        ...
        try {
            ...
            std::string dataFileName = "spline_ball_data_1_" + std::to_string(numPoints) + ".txt";
            std::string gnuplotScriptName = "plot_ball_1_" + std::to_string(numPoints) + ".gp";
            std::string outputImageName = "B" + std::to_string(1) + "r4spline_ball_" + std::to_string(numPoints) + ".png";
            ...
            gnuplotScript << "splot '" << dataFileName << "' with lines title 'r_4 (" << numPoints << " points), BType" << 1 << "'\n";
            gnuplotScript.close();
